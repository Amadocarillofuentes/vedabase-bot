#updated code (07) on 4/21/25 7:48pm:
import discord
...
# --- Merged Verses Conflict Handling ---
conflicts = {
...
# --- URL Resolution Function with Conflict Handling ---
def resolve_url(scripture, chapter, verse):
    """
    Resolves the correct URL for a verse, including merged verses based on conflicts.
    """
    if scripture in conflicts and chapter in conflicts[scripture]:
        for verse_range in conflicts[scripture][chapter]:
            if verse in range(verse_range[0], verse_range[1] + 1):
                # If the verse is within a conflict range, use the range in the URL
